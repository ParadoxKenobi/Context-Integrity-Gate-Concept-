...
The `context_gate` package provides a deterministic gate implementation that follows the concepts above:

- **Canonicalization**: Unicode NFC normalization, whitespace collapse, lowercase casing, and deterministic coercion of numbers and booleans.
- **Validation**: Explicit, named rules that reject on failure with clear error messages.
- **Policy enforcement**: Deterministic allow/deny checks that must pass before persistence.
- **Persistence**: A thread-safe, in-memory store to keep trusted context records.
...
Run the deterministic pipeline tests with:
